...
it('shows the selectable catalog without extra notices when credentials are missing', async () => {
  const service = aiServices.find(item => item.id === 'xai')!;
  ...
  expect(document.body.textContent).not.toContain('API 키를 입력');
  expect(document.body.textContent?.startsWith(service.models[0]!.name)).toBe(true);
  expect(document.body.textContent).not.toMatch(/공식 문서 목록|개 항목/);
