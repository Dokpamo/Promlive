...
import {historyListLayout} from '../src/features/chat/historyListMotion';
...
it('keeps stable row keys and includes the separator gap in pin and unpin destinations', () => {
  ...
  const original = historyListLayout([a, b, c], 48, 15);
  const pinned = historyListLayout([{...c, pinnedAt: 1}, a, b], 48, 15);
  ...
  expect(historyListLayout([a, b, c], 48, 15)).toEqual(original);
});
...
it('does not reserve separator space when a filtered list has only pinned or only unpinned rooms', () => {
  for (const rooms of [[room('a', 1), room('b', 2)], [room('a'), room('b')]]) {
    const layout = historyListLayout(rooms, 48, 15);
    expect(layout.find(item => item.kind === 'divider')).toMatchObject({visible: false, height: 0});
    expect(layout.filter(item => item.kind === 'conversation').map(item => item.top)).toEqual([0, 48]);
  }
});
