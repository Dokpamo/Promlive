...
async function main() {
  ...
  for(const file of await fs.readdir(source,{recursive:true,withFileTypes:true})) {
    ...
    const target=path.join(root,relative.replaceAll('MyApp','Promlive').replace('_gitignore','.gitignore').replace('NuGet_Config','NuGet.config'));
