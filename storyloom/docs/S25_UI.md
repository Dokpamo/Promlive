...
채팅 화면에서 오른쪽으로 쓸면 화면 전체가 둥근 패널처럼 이동하며 왼쪽 채팅 내역이 드러납니다. 내역에서 왼쪽으로 쓸면 채팅으로 돌아옵니다. 이동 중에는 손가락을 따라가며, 짧고 느린 드래그는 원래 위치로 복귀하고 빠른 스와이프는 진행 방향으로 전환합니다. 메뉴 버튼으로 열기, 밀려난 채팅 화면을 눌러 닫기, Android 뒤로가기와 웹 Escape도 지원합니다.
