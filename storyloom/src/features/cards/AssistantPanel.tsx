...
export function AssistantPanel({workspace: w}: {workspace: Workspace}) {
  const editor = w.editor!; const {creation, repo, provider} = w.runtime;
  useSyncExternalStore(creation.subscribe, creation.snapshot);
  const form = w.assistantForm(editor.card.id);
  const {mode, prompt, content, target} = form;
  const setMode = (value: Draft['kind']) => w.editAssistant(editor.card.id, {mode: value});
  const setPrompt = (value: string) => w.editAssistant(editor.card.id, {prompt: value});
  const setContent = (value: string) => w.editAssistant(editor.card.id, {content: value});
  const setTarget = (value: keyof World) => w.editAssistant(editor.card.id, {target: value});
  const [history, setHistory] = useState<Draft[]>([]);
  const live = creation.draft(editor.card.id);
  const draft = live && live.status !== 'applied' ? live : history.find(d => d.status !== 'applied');
  useEffect(() => { let active = true; void repo.drafts(editor.card.id).then(d => {if (active) setHistory(d);}).catch(e => w.report(e)); return () => {active = false;}; }, [repo, editor.card.id, editor.card.revision, w]);
  useEffect(() => {
    if (draft && (form.draftId !== draft.id || form.sourceContent !== draft.content)) w.editAssistant(editor.card.id, {draftId: draft.id, sourceContent: draft.content, content: draft.content});
  }, [draft, form.draftId, form.sourceContent, editor.card.id, w]);
  ...
  return <ScrollView keyboardShouldPersistTaps="handled" style={{flex: 1, backgroundColor: '#F6F4F8'}} contentContainerStyle={{padding: 23, gap: 21}}>
    ...
    {draft?.status === 'generating' ? <Button variant="secondary" icon="stop" onPress={() => creation.cancel(draft.id)}>생성 중단</Button> : <Button icon="spark" disabled={!provider.connected || !prompt.trim()} onPress={() => {
      if (editor.dirty) { w.report(new Error('먼저 카드를 저장해 주세요. 생성 중에도 자유롭게 편집할 수 있어요.')); return; }
      run(creation.generateDraft(editor.card, prompt, mode));
    }}>{mode === 'writing' ? '초안 만들기' : '자료 조사하기'}</Button>}
    {!provider.connected && <View style={{backgroundColor: '#EBE6EF', borderRadius: 9, padding: 14, gap: 8}}><Text style={{fontSize: 12, color: colors.accent, fontWeight: '600'}}>AI 연결을 기다리고 있어요</Text><Text style={[styles.small, {fontSize: 11}]}>지금은 직접 작성하고 저장할 수 있어요.{ '\n'}Grok 연결은 테스트 단계에서 진행할 예정이에요.</Text><Button variant="ghost" small onPress={() => run(w.go('settings'))} style={{alignSelf: 'flex-start', paddingHorizontal: 0}}>연결 설정 보기 ↗</Button></View>}
