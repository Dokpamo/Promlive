...
export function HeaderButton({width, icon, label, onPress, testID, leading = false, grouped = false}: {
  ...
  return <Pressable testID={testID} accessibilityRole="button" accessibilityLabel={label} onPress={onPress} onPressIn={onPressIn} onPressOut={onPressOut} style={{
    width: buttonWidth,
    height,
    flexShrink: 0,
    marginRight: leading ? (r.backGap - r.gap) * s : 0,
    borderRadius: grouped ? 0 : height / 2,
    backgroundColor: grouped ? 'transparent' : c.header,
    ...
    <Animated.View testID="header-button-content" style={{width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', transform: [{scale: progress.interpolate({inputRange: [0, 1], outputRange: [1, 0.98]})}]}}>
      <Animated.View testID="header-button-highlight" pointerEvents="none" style={{position: 'absolute', left: (buttonWidth - highlightSize) / 2, top: (height - highlightSize) / 2, width: highlightSize, height: highlightSize, borderRadius: highlightSize / 2, backgroundColor: grouped ? c.actionPressed : c.headerPressed, opacity: progress}}/>
