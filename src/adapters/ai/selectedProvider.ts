...
export class SelectedProvider implements AiProvider {
  ...
  async *stream(request: AiRequest, signal: AbortSignal) {
    ...
    const provider = new GrokProvider({model, credentialReference, inputCharacterLimit: this.inputCharacterLimit}, {get: async reference => reference === credentialReference ? key : null}, this.transport);
