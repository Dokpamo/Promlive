...
export function expandedComposerFrame(viewport: Viewport, insets: Insets): ComposerFrame {
  ...
  const inset = settingsReference.sheetInset * scale;
  const bottom = Math.max(insets.bottom, inset);
  const width = Math.min(settingsReference.contentMaxWidth, viewport.width - insets.left - insets.right - 2 * inset);
  const height = Math.max(0, Math.min(viewport.height * 0.9, viewport.height - insets.top - inset - bottom));
  return {x: insets.left + (viewport.width - insets.left - insets.right - width) / 2, y: viewport.height - bottom - height, width, height, radius: settingsReference.radius * scale};
}
