...
import {folderPath, type FolderTree} from './folderTree';
...
export function useFolderNavigation({value, initialFolderId, blocked, canVisit, width}: {
  value: Pick<FolderTree, 'folders'>; initialFolderId: string | null; blocked: boolean;
  canVisit: (id: string | null) => boolean; width: number;
}) {
  const [route, setRoute] = useState<Route>(() => {
    const trail = [null, ...folderPath(value, initialFolderId).map(folder => folder.id)];
