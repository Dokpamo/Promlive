...
interface EditorScreenProps {
  ...
  startChat: (card: Card) => Promise<void>; duplicate: (card: Card) => Promise<void>; archive: (card: Card) => Promise<void>;
      ...
      {!wide && showAi ? <AssistantPanel session={session} creation={creation} connected={creation.coordinator.provider.connected} openSettings={openSettings} report={report}/> : <ScrollView keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" automaticallyAdjustKeyboardInsets contentContainerStyle={{padding: compact ? 22 : 34, paddingBottom: 50}}>
        <View style={{maxWidth: 770, width: '100%', alignSelf: 'center', gap: 27}}>
          ...
          {card.body.kind === 'template' ? <>
            ...
            {tab === '세계관' && <View style={{gap: 24}}><Section number="01" title="어떤 세계인가요?" detail="이야기가 펼쳐질 공간과 그곳만의 규칙을 정해 보세요."/><Field label="세계관" value={card.body.data.world} onChangeText={v => session.editWorld('world', v)} multiline placeholder="이 세계는 어떤 모습인가요? 사람들은 무엇을 믿고, 어떻게 살아가나요?" style={{minHeight: 170}} maxLength={30000}/><Field label="시대와 장소" value={card.body.data.era} onChangeText={v => session.editWorld('era', v)} placeholder="예: 계절을 잊은 현대의 도시, 자정의 도서관" multiline style={{minHeight: 76}} maxLength={30000}/><Field label="세계의 규칙" value={card.body.data.rules} onChangeText={v => session.editWorld('rules', v)} multiline placeholder="가능한 것과 불가능한 것, 꼭 지켜야 하는 약속을 적어 주세요." hint="AI가 대화 중 일관되게 지켜야 할 설정이에요." maxLength={30000}/></View>}
            {tab === '등장인물' && <View style={{gap: 24}}><Section number="02" title="누구를 만나게 되나요?" detail="작은 습관부터 말투까지, 인물에게 생기를 불어넣어 주세요."/><Field label="등장인물 이름" value={card.body.data.characterName} onChangeText={v => session.editWorld('characterName', v)} maxLength={100} placeholder="어떻게 불러 주면 좋을까요?"/><Field label="역할과 배경" value={card.body.data.role} onChangeText={v => session.editWorld('role', v)} multiline maxLength={30000}/><Field label="성격과 말투" value={card.body.data.personality} onChangeText={v => session.editWorld('personality', v)} multiline maxLength={30000}/><Field label="사용자와의 관계" value={card.body.data.relationship} onChangeText={v => session.editWorld('relationship', v)} multiline maxLength={30000}/><Field label="첫 만남의 장면" value={card.body.data.greeting} onChangeText={v => session.editWorld('greeting', v)} multiline style={{minHeight: 150}} maxLength={30000}/></View>}
            {tab === '대화 스타일' && <View style={{gap: 24}}><Section number="03" title="이야기의 온도를 정해요" detail="어떤 분위기로, 어떤 속도로 이야기를 이어갈까요?"/><Field label="장르" value={card.genre} onChangeText={genre => session.edit({genre})} maxLength={40}/><Field label="대화 지침" value={card.body.data.tone} onChangeText={v => session.editWorld('tone', v)} multiline style={{minHeight: 180}} placeholder="문체, 응답 길이, 묘사 방식 등을 자유롭게 적어 주세요." maxLength={30000}/></View>}
          </> : <View style={{gap: 20}}>
            ...
            {preview > 0 && <CodePreview key={preview} card={runningCard} coordinator={creation.coordinator} allowed={allowed}/>}
          </View>}
          <View style={{height: 1, backgroundColor: colors.line}}/>
          <View style={[styles.row, {justifyContent: 'space-between', flexWrap: 'wrap', gap: 8}]}><Button small variant="ghost" icon="copy" onPress={() => run(duplicate(card))}>카드 복제</Button><Button small variant="ghost" icon="archive" onPress={() => run(archive(card))}>{card.archived ? '서재로 가져오기' : '보관함으로 이동'}</Button></View>
        </View>
      </ScrollView>}
    </View>
