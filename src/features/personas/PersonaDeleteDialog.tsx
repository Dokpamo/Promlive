...
export function PersonaDeleteDialog({title, detail, onClose, onDelete}: {
  title: string; detail: string; onClose: () => void; onDelete: () => Promise<void>;
}) {
  ...
  return <Modal visible transparent animationType="none" statusBarTranslucent navigationBarTranslucent onRequestClose={() => {}} onShow={() => syncSystemBars(isDark)}>
    <View testID="persona-delete-overlay" accessibilityViewIsModal onAccessibilityEscape={() => {}} style={{flex: 1, justifyContent: 'center', alignItems: 'center', paddingTop: safe.top, paddingBottom: safe.bottom}}>
      ...
      <Animated.View testID="persona-delete-confirm" style={{width: Math.min(340, (width - safe.left - safe.right) * 0.82), maxHeight: (height - safe.top - safe.bottom) * 0.8,
