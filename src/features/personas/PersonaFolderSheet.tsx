...
import {canMovePersonaFolders, nextPersonaFolderName, personaEntryOrder, personaFolderPath, type PersonaCollection, type PersonaPreferences} from './personaPreferences';
import {usePersonaFolderNavigation} from './usePersonaFolderNavigation';

/** One destination picker serves the item menu and the selection bar. */
export function PersonaFolderSheet({ids, folderIds, initialFolderId, value, store, onClose, onMoved}: {
  ids: string[]; folderIds: string[]; value: PersonaCollection; store: PersonaPreferences;
  ...
  const navigation = usePersonaFolderNavigation({value, initialFolderId, blocked,
    canVisit: id => canMovePersonaFolders(value, folderIds, id), width: Math.min(560, width - 2 * g.sheetInset * s)});
  const {folderId} = navigation;
  // Selected descendants travel with their parent; only independent roots move.
  const locations = [
    ...value.items.filter(item => ids.includes(item.id) && canMovePersonaFolders(value, folderIds, item.folderId)).map(item => item.folderId),
    ...value.folders.filter(item => folderIds.includes(item.id) && canMovePersonaFolders(value, folderIds, item.parentId)).map(item => item.parentId),
  ];
  const canMove = canMovePersonaFolders(value, folderIds, folderId) && locations.some(id => id !== folderId);
  ...
  const renderFolder = (id: string | null, interactive: boolean) => {
    ...
    const parentName = value.folders.find(folder => folder.id === currentFolder?.parentId)?.name ?? '페르소나';
    const destinations = value.folders.filter(folder => folder.parentId === id && canMovePersonaFolders(value, folderIds, folder.id))
    ...
    return <View onLayout={event => {const height = event.nativeEvent.layout.height; setPageHeight(old => Math.max(old, height));}}>
      ...
      <RowPressable accessibilityRole="button" accessibilityLabel="새 폴더" disabled={disabled || !canMovePersonaFolders(value, folderIds, id)} onPress={() => {
        if (pending.current || moved.current || disabled) return;
        namingOpen.current = true;
        setCreating({parentId: id, name: nextPersonaFolderName(value.folders.filter(folder => folder.parentId === id))});
  ...
  return <SettingsSheet title="폴더" contentKey={folderId} horizontalDrag={navigation.drag} onBackRequest={navigation.back}
    dismiss={dismissing} obscured={!!creating} overlay={creating && <ItemRenameSheet scope="persona-folder" item={{title: creating.name}} heading="새 폴더" inputLabel="폴더 이름" maxLength={40}
  ...
  </>}>{() => <View testID="persona-folder-picker">
    <PersonaBreadcrumbs path={personaFolderPath(value, folderId)} scale={s} onNavigate={navigate} disabled={blocked || navigation.dragging}
      labelPrefix="이동 위치, " testID="persona-folder-picker-path"/>
    <View testID="persona-folder-pages" style={{marginHorizontal: -g.sheetPadding * s, overflow: 'hidden', minHeight: pageHeight}}>
