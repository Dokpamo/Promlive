...
export function PersonaEditorSheet({item, store, onClose, onCreated}: Props) {
  ...
  return <SwipeBackModal sheet sheetHeight={height} active={!saving} onClose={onClose} onDismissStart={() => {
  ...
  }}>{(close, motionStyle) => <SettingsTextEditorHost><KeyboardMotionProvider>
    <PersonaEditorSurface draft={draft} editing={!!item} closing={closing} saving={saving} obscured={!!photo} error={error} input={input} keyboardVisible={keyboardVisible}
      change={change} retry={() => change(current.current)} onPhoto={setPhoto} onPhotoError={setError} close={close} create={() => {void create(close);}} motionStyle={motionStyle} onHeight={setHeight}/>
...
function PersonaEditorSurface({draft, editing, closing, saving, obscured, error, input, keyboardVisible, change, retry, onPhoto, onPhotoError, close, create, motionStyle, onHeight}: {
  ...
  onPhoto: (photo: ProfilePhoto) => void; onPhotoError: (error: string) => void; close: () => void; create: () => void;
    ...
    <KeyboardDock fraction={fraction} bottomInset={insets.bottom} freezeKeyboard={false} followCaret={false}>
      <Animated.View testID="persona-editor" accessibilityViewIsModal pointerEvents={obscured ? 'none' : 'auto'} accessibilityElementsHidden={obscured} importantForAccessibility={obscured ? 'no-hide-descendants' : 'auto'} onLayout={event => onHeight(event.nativeEvent.layout.height + bottom)}
        ...
        <ScreenHeader width={width} edgeTint={false}>
          ...
          {editing ? <View style={{width: referenceHeader.height * s}}/> : <SwipeBackBoundary><HeaderButton width={width} icon="check" label="페르소나 만들기" disabled={saving || closing || !draft.name.trim()} onPress={create}/></SwipeBackBoundary>}
        </ScreenHeader>
        ...
        <SwipeBackBoundary style={{marginHorizontal: g.rowInset * s, marginTop: 16 * s, marginBottom: 14 * s}}>
          ...
          <TextInput ref={input} testID="persona-name-input" accessibilityLabel="페르소나 이름" value={draft.name} onChangeText={name => change({name})} editable={!closing && !saving} autoFocus selection={selection}
